...
int main(){
    ...
    //Input position of bit to set
    printf("Enter position of bit to set:");
    //Set input to address of position variable
    scanf("%d", &bitposition);

    //Perform bit setting using binary or operator | between the number and the binary value of number with 1 left shifted by the value of bitposition
